...
const amplitudeEndPoint = Cypress.env('amplitudeEndPoint')
...
describe('Create a new User to test Subscription Page', () => {
  ...
  it('should verify that user is able to subscribe to "TIME TRACKING PLAN - MONTHLY"', () => {
    ...
    cy.get(PLAN_DROPDOWN_LIST)
      .should('be.visible')
      .contains('Monthly Time Tracking Plan')
      .click()
    ...
    cy.intercept('POST', amplitudeEndPoint).as('subscribePlan')
  ...
  it('should verify that user is successfully subscribe to "TIME TRACKING PLAN - MONTHLY"', () => {
    // Logins the newly created user
    cy.visit(`${baseUrl}/login`)
    cy.get(LOGIN_EMAIL_FLD).should('be.visible').type(NEW_SIGNUP_EMAIL)
    cy.get(LOGIN_PASSWORD_FLD).type(DEFAULT_PASSWORD)
    ...
    // Verify that the user is subscribe to "TIME TRACKING PLAN MONTHLY"
